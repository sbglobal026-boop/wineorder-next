...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  return {
    title,
    description,
    alternates: { canonical: `/events/food/${id}` },
    openGraph: {
      type: 'website',
      title,
      description,
...
export default async function FoodDetailLayout({
  children,
  params,
}: {
    ...
    <>
      ...
      {data && (
        <script
          ...
          dangerouslySetInnerHTML={{ __html: JSON.stringify(productJsonLd(data as ProductSeoRow, 'food')) }}
        />
      )}
      {children}
    </>
