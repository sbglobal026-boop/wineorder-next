  ...
  return (
    <div className="min-h-screen" style={pageBg}>
      ...
      <header className="max-w-[760px] mx-auto text-center px-5 pt-16 md:pt-20 pb-6">
        <p className="text-[13px] tracking-[0.28em] uppercase text-[#0e3719] mb-3.5">Cart</p>
        ...
        <div className="grid lg:grid-cols-[1fr_360px] gap-8 items-start">
          ...
          <div className="flex flex-col gap-4">
            {items.map(({ productId, qty, product }) => {
              ...
              return (
                <div key={productId} className="cutecard rounded-[24px] border border-[#eae7e7] bg-[#FFFFFF] p-4 md:p-5 flex items-center gap-4 md:gap-5">
                  {/* 이미지 */}
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[11px] tracking-[0.16em] uppercase text-[#0e3719] mb-1">{product.category} · {product.origin}</p>
                    <Link href={href} className="no-underline">
                      <h4 className="font-[family-name:var(--font-playfair-display)] text-[18px] md:text-[20px] text-[#1C1A17] leading-tight mb-2 line-clamp-2 hover:text-[#0e3719] transition-colors">{product.name}</h4>
                    </Link>
                    {/* 수량 알약 */}
                    <div className="flex items-center rounded-full border border-[#d7d3d3] w-fit overflow-hidden">
                      <button onClick={() => updateCartQty(productId, qty - 1)} className="w-9 h-9 flex items-center justify-center text-[#605d5d] hover:text-[#0e3719] hover:bg-[#0e3719]/[0.06] transition-colors">−</button>
            ...
            <div className="mt-2">
              ...
              <textarea
                ...
                rows={3}
                className="w-full rounded-2xl border border-[#eae7e7] bg-[#FFFFFF] px-4 py-3 text-sm text-[#605d5d] placeholder-[#bab6b6] resize-none focus:outline-none focus:border-[#5C7A63] transition-colors"
              />
            </div>
          </div>
          ...
          <div className="lg:sticky lg:top-[90px] rounded-[24px] border border-[#eae7e7] bg-[#FFFFFF] p-6 md:p-7">
            ...
            <Link
              href="/events/wines"
              className="block text-center rounded-full border border-[#d7d3d3] text-[#605d5d] hover:border-[#5C7A63] hover:text-[#0e3719] text-sm py-3 transition-colors no-underline"
            >
              계속 쇼핑하기
            </Link>
            <button
              onClick={clearCart}
              className="w-full text-xs text-[#bab6b6] hover:text-[#0e3719] transition-colors py-3"
            >
              전체 비우기
            </button>
          </div>
        ...
        {recommended.length > 0 && (
          <section className="mt-20 md:mt-28">
            <div className="flex items-baseline justify-between mb-6">
              ...
              <Link href="/events/wines" className="text-sm text-[#0e3719] no-underline border-b border-[#5C7A63]">전체보기 →</Link>
            </div>
